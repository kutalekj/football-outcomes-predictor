...
# -----------------------------
COMP_INFO = {
    39: {"name": "Premier League", "fs_alias": "Premier League"},
    40: {"name": "Championship", "fs_alias": "Championship"},
    41: {"name": "League One", "fs_alias": "EFL League One"},
    42: {"name": "League Two", "fs_alias": "EFL League Two"},
    61: {"name": "Ligue 1", "fs_alias": "Ligue 1"},
    62: {"name": "Ligue 2", "fs_alias": "Ligue 2"},
    78: {"name": "Bundesliga", "fs_alias": "Bundesliga"},
    79: {"name": "2. Bundesliga", "fs_alias": "2. Bundesliga"},
    88: {"name": "Eredivisie", "fs_alias": "Eredivisie"},
    94: {"name": "Primeira Liga", "fs_alias": "Liga NOS"},
    106: {"name": "Ekstraklasa", "fs_alias": "Ekstraklasa"},
    119: {"name": "Superliga", "fs_alias": "Superliga"},
    135: {"name": "Serie A", "fs_alias": "Serie A"},
    136: {"name": "Serie B", "fs_alias": "Serie B"},
    140: {"name": "La Liga", "fs_alias": "La Liga"},
    141: {"name": "Segunda División", "fs_alias": "Segunda División"},
    144: {"name": "Jupiler Pro League", "fs_alias": "Pro League"},
    179: {"name": "Premiership", "fs_alias": "Premiership"},
    188: {"name": "A-League", "fs_alias": "A-League"},
    203: {"name": "Süper Lig", "fs_alias": "Süper Lig"},
    207: {"name": "Super League", "fs_alias": "Super League"},
    218: {"name": "Bundesliga", "fs_alias": "Bundesliga"},
    307: {"name": "Pro League", "fs_alias": "Professional League"},
    323: {"name": "Indian Super League", "fs_alias": "Indian Super League"},
}
...
# -----------------------------
# PARSING mp_out.txt (with seasons)
# -----------------------------
def parse_mp_out(path: Path):
    """
    Parse mp_out.txt into a dict of dicts with seasons:
    {
        "mp3_all_players_involved_in_team_strength_calculation": {
            season: {comp_id: value, ...},
            ...
        },
        ...
    }
    ...
    entry_pattern = re.compile(r"^\s+(\d+)\s*/\s*(\d+):\s+(\d+)\s*$")
        ...
        for line in f:
            ...
            if current_key is not None:
                ...
                if m_entry:
                    ...
                    season = int(m_entry.group(2))
                    value = int(m_entry.group(3))
                    metrics[current_key].setdefault(season, {})
                    metrics[current_key][season][comp_id] = value
...
# Convenience references (now: dict[season][comp_id] = value)
...
# Seasons and competitions (global order)
SEASONS = sorted(mp3.keys())  # e.g. [2021, 2022, 2023, 2024, 2025]
comp_ids = sorted({cid for season in SEASONS for cid in mp3[season].keys()})
...
def annotate_total_sum(ax, total_value: int, fontsize=9):
...
def setup_axes(ax, title: str, show_xlabel: bool = True):
...
def plot_chart_1():
    ...
    # Compute global max for consistent y-scale
    max_val = 0
    for season in SEASONS:
        totals_s = [mp3[season].get(cid, 0) for cid in comp_ids]
        if totals_s:
            max_val = max(max_val, max(totals_s))

    for idx, season in enumerate(SEASONS):
        ax = axes[idx]
        totals = [mp3[season].get(cid, 0) for cid in comp_ids]
        ...
        show_xlabel = idx == len(SEASONS) - 1
        setup_axes(
            ax,
            f"1) Total players involved in team strength calculation – Season {season}",
...
def plot_chart_2():
    ...
    # global y max over seasons
    max_val = 0
    for season in SEASONS:
        total_s = [mp3[season].get(cid, 0) for cid in comp_ids]
        if total_s:
            max_val = max(max_val, max(total_s))

    for idx, season in enumerate(SEASONS):
        ax = axes[idx]
        total = [mp3[season].get(cid, 0) for cid in comp_ids]
        missing_dob = [mp5.get(season, {}).get(cid, 0) for cid in comp_ids]
        ...
        ax.bar(
            ...
            color=[COMP_COLORS.get(cid, "gray") for cid in comp_ids],
            edgecolor="black",
            label="Date of birth known" if idx == 0 else None,
        )
        ax.bar(
            x,
            missing_dob,
            ...
            color="black",
            edgecolor="black",
            label="Dates of birth missing (mp5)" if idx == 0 else None,
        )
        ...
        setup_axes(
            ax,
            f"2) Dates of birth missing vs total (mp5 vs mp3) – Season {season}",
            show_xlabel=show_xlabel,
        )
...
def plot_chart_3():
    ...
    for idx, season in enumerate(SEASONS):
        ax = axes[idx]
        total = [mp3[season].get(cid, 0) for cid in comp_ids]
        missing_match = [mp6.get(season, {}).get(cid, 0) for cid in comp_ids]
        ...
        ax.bar(
            ...
            color=[COMP_COLORS.get(cid, "gray") for cid in comp_ids],
            edgecolor="black",
            label="Matched players" if idx == 0 else None,
        )
        ax.bar(
            x,
            missing_match,
            ...
            color="black",
            edgecolor="black",
            label="FS–SF name matching misses (mp6)" if idx == 0 else None,
        )
        ...
        setup_axes(
            ax,
            f"3) FS–SF matching misses vs total (mp6 vs mp3) – Season {season}",
            show_xlabel=show_xlabel,
        )
        annotate_total_sum(ax, sum(total))
...
def plot_chart_4():
    ...
    for idx, season in enumerate(SEASONS):
        ...
        ax.bar(
            ...
            color=[COMP_COLORS.get(cid, "gray") for cid in comp_ids],
            edgecolor="black",
            label="Players used" if idx == 0 else None,
        )
        ax.bar(
            x,
            missing_dob,
        ...
        ax.bar(
            ...
            color="darkgray",
            edgecolor="black",
            label="FS–SF matching miss (mp6)" if idx == 0 else None,
        )
        ...
        setup_axes(
            ax,
            f"4) Combined missing: dates of birth + matching (mp5 + mp6) vs total (mp3) – Season {season}",
            show_xlabel=show_xlabel,
        )
        annotate_total_sum(ax, sum(total))
...
def plot_chart_5():
    ...
    for idx, season in enumerate(SEASONS):
        ax = axes[idx]
        total = [mp3[season].get(cid, 0) for cid in comp_ids]
        imputed = [mp8b.get(season, {}).get(cid, 0) for cid in comp_ids]
        ...
        ax.bar(
            ...
            color=[COMP_COLORS.get(cid, "gray") for cid in comp_ids],
            edgecolor="black",
            label="Non-imputed players" if idx == 0 else None,
        )
        ax.bar(
            x,
            imputed,
            ...
            color="black",
            edgecolor="black",
            label="Imputed players (mp8b)" if idx == 0 else None,
        )
        ...
        setup_axes(
            ax,
            f"5) Imputed players due to missing data (mp8b) vs total (mp3) – Season {season}",
            show_xlabel=show_xlabel,
        )
        annotate_total_sum(ax, sum(total))
...
def plot_chart_6():
    ...
    for idx, season in enumerate(SEASONS):
        ...
        missing_dob = [mp5.get(season, {}).get(cid, 0) for cid in comp_ids]
        missing_match = [mp6.get(season, {}).get(cid, 0) for cid in comp_ids]
        missing_skill = [mp7.get(season, {}).get(cid, 0) for cid in comp_ids]
        ...
        ax.bar(
            ...
            color="white",
            edgecolor="black",
            label="No CSV in range (mp7)" if idx == 0 else None,
        )
        ...
        setup_axes(
            ax,
            f"6) Causes of missing data (dates of birth + matching + no CSV) vs total (mp3) – Season {season}",
            show_xlabel=show_xlabel,
        )
        annotate_total_sum(ax, sum(total))
...
def plot_chart_7():
    ...
    # Global max over all seasons/bar totals for y-limits
    max_val = 0
    for season in SEASONS:
        for cid in comp_ids:
            f2g = mp9_field_to_gk.get(season, {}).get(cid, 0)
            g2d = mp9_gk_to_def.get(season, {}).get(cid, 0)
            g2m = mp9_gk_to_mid.get(season, {}).get(cid, 0)
            g2a = mp9_gk_to_att.get(season, {}).get(cid, 0)
            total_bar = f2g + g2d + g2m + g2a
            max_val = max(max_val, total_bar)
    ...
    for idx, season in enumerate(SEASONS):
        ...
        field_to_gk = [mp9_field_to_gk.get(season, {}).get(cid, 0) for cid in comp_ids]
        gk_to_def = [mp9_gk_to_def.get(season, {}).get(cid, 0) for cid in comp_ids]
        gk_to_mid = [mp9_gk_to_mid.get(season, {}).get(cid, 0) for cid in comp_ids]
        gk_to_att = [mp9_gk_to_att.get(season, {}).get(cid, 0) for cid in comp_ids]
        ...
        ax.bar(
            ...
            color=c_field_to_gk,
            edgecolor="black",
            label="field → GK" if idx == 0 else None,
        )
        ...
        ax.bar(
            ...
            color=c_gk_to_def,
            edgecolor="black",
            label="GK → DEF" if idx == 0 else None,
        )
        ...
        ax.bar(
            ...
            color=c_gk_to_mid,
            edgecolor="black",
            label="GK → MID" if idx == 0 else None,
        )
        ...
        ax.bar(
            ...
            color=c_gk_to_att,
            edgecolor="black",
            label="GK → ATT" if idx == 0 else None,
        )
        ...
        show_xlabel = idx == len(SEASONS) - 1
        ax.set_title(
            f"7) Position balancing after imputation (mp9_*) – Season {season}",
            fontsize=10,
            fontweight="bold",
        )
        if show_xlabel:
