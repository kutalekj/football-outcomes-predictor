...
from football_outcomes.training.train_ann import get_embedding_extractor, normalize_embeddings
...
while True:
    ...
    for comp in global_instance.all_comps:
        if len(comp.regular_round_keywords) > 0:
            ...
            for fixture in data_fixtures["response"]:
                ...
                if (
                    new_match.home_team_lineup is not None
                    and new_match.away_team_lineup is not None
                    and len(new_match.home_team_lineup) > 0
                    and len(new_match.away_team_lineup) > 0
                ):
                ...
                # TODO: I think the time zones are still not handled correctly here...
                    ...
                    home_team_prev_match = ut.get_previous_match(
                        new_match,
                        new_match.home_team.id,
                        same_comp=True,
                        same_season=False,
                        regular=True,
                    )
                    home_team_prev_af_lineup = (
                        home_team_prev_match.home_team_lineup
                        if home_team_prev_match.home_team.id == new_match.home_team.id
                        else home_team_prev_match.away_team_lineup
                    )

                    away_team_prev_match = ut.get_previous_match(
                        new_match,
                        new_match.away_team.id,
                        same_comp=True,
                        same_season=False,
                        regular=True,
                    )
                    away_team_prev_af_lineup = (
                        away_team_prev_match.away_team_lineup
                        if away_team_prev_match.away_team.id == new_match.away_team.id
                        else away_team_prev_match.home_team_lineup
                    )
                    ...
                    log_str = (
                        f"\t\t\t\t\tAF lineups taken from previous matches for teams "
    ...
    for match in matches_to_predict:
        try:
            if (
                match.home_fs_team_lineup is not None
                and match.away_fs_team_lineup is not None
                and len(match.home_fs_team_lineup) == 0
                and len(match.away_fs_team_lineup) == 0
            ):
                print("")
            ...
            home_strength_norm = np.squeeze(
                ut.separate_normalize_gk_and_outfield_skills(np.expand_dims(home_strength, axis=0)),
                axis=0,
            )  # TODO: Error once occurred here - home_strength not having enough dimensions
            away_strength_norm = np.squeeze(
                ut.separate_normalize_gk_and_outfield_skills(np.expand_dims(away_strength, axis=0)),
                axis=0,
            )
            ...
            home_id_embedding = np.squeeze(
                team_id_embedding_model.predict(home_id_array), axis=1
            )  # (num_samples = 1, 8)
