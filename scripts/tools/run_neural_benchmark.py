...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=(
            "Run the Step 8 chronological neural benchmark against the "
            "validated frozen snapshot. By default every eligible rolling "
            "fold is evaluated."
        )
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        type=Path,
        required=True,
        help="Parent directory for the manifest-backed benchmark run.",
    )
    ...
    parser.add_argument(
        ...
        default=None,
        help=(
            "Optional partial-run fold count. Omit this option for the full "
            "benchmark through the final eligible round."
        ),
    )
...
def main() -> int:
    ...
    run_directory = run_neural_benchmark(
