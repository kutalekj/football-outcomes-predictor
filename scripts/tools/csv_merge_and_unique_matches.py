...
def merge_csv_files_without_duplicates(input_csv_paths, output_csv_path):
    ...
    for csv_path in input_csv_paths:
        ...
        with open(csv_path, mode="r", encoding="utf-8", newline="") as f:
            ...
            if file_header is None:
                ...
                continue
            if header is None:
                header = file_header  # if didn't store a header yet,
                # store the first file's header as the "master" header
            else:
                if file_header != header:
                    print(
                        f"Warning: Header in {csv_path} does not match the primary header. " "Skipping file."
                    )  # file's header doesn't match the master header
                    continue
    ...
    if header is None:
        print(
            "No valid rows/headers found in input files. Output CSV will not be created."
        )  # write all unique rows to the output CSV
        return
