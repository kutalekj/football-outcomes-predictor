...
def train(mapped_team_ids, batch_size, num_epochs):
    ...
    train_ids, val_ids, train_labels, val_labels = train_test_split(
        np.array(mapped_team_ids), np.array(mapped_team_ids), test_size=0.2, random_state=42, shuffle=True)
    ...
    team_embedding = Embedding(input_dim=settings.NUM_REGULAR_TEAMS, output_dim=settings.TEAM_ID_EMBEDDING_SIZE,
                               name='team_embedding')(team_input)
