...
def build_mlp(
    ...
    home_team_strength_input = Input(
        shape=(settings.TEAM_STRENGTH_EMBEDDING_SIZE,),
        dtype="float32",
        name="home_team_strength_input",
    )
    away_team_strength_input = Input(
        shape=(settings.TEAM_STRENGTH_EMBEDDING_SIZE,),
        dtype="float32",
    ...
    merged = Concatenate()(
        [
            ...
            away_team_input,
            comp_input,
            home_team_strength_input,
            away_team_strength_input,
        ]
    )
    ...
    lr_schedule = CustomLRSchedule(lr)  # allow for custom dynamic LR update
...
def train(regular_matches_in_rounds, team_id_map, comp_id_map):
    ...
    for round_number in range(NUM_TRAINING_ROUNDS + 1, total_rounds):
        ...
        train_numerical_features, train_labels = get_data_for_window(
        ...
        (
            ...
            train_away_ids,
            train_comp_ids,
            train_home_strengths,
            train_away_strengths,
        ) = extract_embeddings(
            regular_matches_in_rounds,
            round_number,
            NUM_TRAINING_ROUNDS,
            team_id_embedding_model,
            comp_id_embedding_model,
            team_strength_embedding_model,
            team_id_map,
            comp_id_map,
        )

        val_numerical_features, val_labels = get_data_for_round(regular_matches_in_rounds, round_number)
        val_home_ids, val_away_ids, val_comp_ids, val_home_strengths, val_away_strengths = extract_embeddings(
            regular_matches_in_rounds,
            round_number + 1,
            1,
            team_id_embedding_model,
        ...
        lr_schedule.lr_var.assign(new_lr)

        print(
            f"\t\t\t\t\t\t\tRound {str(round_number)}: {str(train_numerical_features.shape[0])} train and"
            f" {str(val_numerical_features.shape[0])} validation matches (lr={str(new_lr)})"
        )
    ...
    if last_100_rounds_acc < 0.555:
        print(
            f"\tAccuracy ({last_100_rounds_acc:.3f}) in last 100 rounds is too low. Deleting log directory: {log_dir}"
        )
...
def extract_embeddings(
    ...
    return (
        ...
        away_id_embeddings,
        comp_id_embeddings,
        home_strength_embeddings,
        away_strength_embeddings,
    )
...
def get_embedding_extractor(full_model, embedding_layer_name):
    ...
    return Model(inputs=full_model.input, outputs=full_model.get_layer(embedding_layer_name).output)
...
class CustomLRSchedule(tf.keras.optimizers.schedules.LearningRateSchedule):
    def __init__(self, initial_lr):
        super().__init__()
        self.lr_var = tf.Variable(initial_lr, trainable=False)  # store the LR as a tf.Variable so it can be updated

    def __call__(self, step):
        return self.lr_var  # ignore 'step' and simply return the current LR
