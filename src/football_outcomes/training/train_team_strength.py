...
def build_team_strength_autoencoder(
    conv_filters, dense_neurons, dropout, kernel_size, final_embedding_size, input_shape=(11, 34)
):
...
def train(team_player_skills, batch_size=32, num_epochs=100):
    ...
    # Build models
    autoencoder, encoder = build_team_strength_autoencoder(
        conv_filters=256, dense_neurons=256, dropout=0.2, kernel_size=7, final_embedding_size=24
    )
    ...
    return (
        autoencoder,
        encoder,
    )  # encoder can be used to extract team strength embeddings after training
