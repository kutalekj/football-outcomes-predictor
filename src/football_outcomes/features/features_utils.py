...
def calculate_team_strength(curr_match, team_id):
    ...
    global_instance.mp3_all_players_involved_in_team_strength_calculation[curr_match.comp.id][curr_match.season] += 11
    ...
    if len(team_fs_lineup) == 0:
        ...
        # TODO manual output check: count how many such missing ones are there in total -
        #  hopefully very few!!!
        # TODO: Players_check 4: count number of calls of this complete team imitation (BAD THING)
        #  - should be same number as mp1b???
        global_instance.mp4_team_strength_complete_lineup_imitation[curr_match.comp.id][curr_match.season] += 11
    ...
    # Iterate over FS lineup players
        ...
        for fs_player in team_fs_lineup:
            ...
            sf_player_id, sf_player_name, sf_player_full_name = ut.match_fs_player_to_sf_players(
                fs_player, sf_players_with_same_dob, curr_match.comp.id, curr_match.season
            )
        ...
        if len(team_sf_players_skills) != 11:
            print(
                f"There are {11 - len(team_sf_players_skills)} player skills missing for "
                f"[{team_name}]! Imitating..."
                f"({curr_match.home_team.name} vs. {curr_match.away_team.name} played at "
                f"{curr_match.datetime})"
            )
            # TODO: Players_check 8: count number of calls of this additional imitation (BAD THING)
            #  - this should equal to mp5 + mp6 + mp7?
            global_instance.mp8a_team_strength_imitated_players_as_no_CSV_data[curr_match.comp.id][
                curr_match.season
            ] += 1
