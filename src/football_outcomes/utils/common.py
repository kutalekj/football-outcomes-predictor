...
def get_n_previous_matches(n, curr_match, team_id, home_away=None, same_comp=False, same_season=False, regular=False):
    ...
    if home_away == "home":
        ...
        for i in range(0, n):
            prev_home_match = get_last_home_away_match(
                curr_prev_match, team_id, "home", same_comp, same_season, regular
            )
    ...
    # Get N last away matches
        ...
        for i in range(0, n):
            prev_away_match = get_last_home_away_match(
                curr_prev_match, team_id, "away", same_comp, same_season, regular
            )
...
def get_all_regular_matches_in_season_table_up_to_date(curr_season_table, date):
    ...
    for team in curr_season_table.teams:
        ...
        team_matches = [
            ...
            if match.season == curr_season_table.season and match.datetime < date and match.round.is_regular
        ]
...
def get_sf_player_data(match_datetime, sf_player_id, team_season_info, fs_position, comp_id):
    global_instance = Global.get_instance()
    ...
    if len(sf_player_available_csv_files) == 0:
        raise ValueError(
            f"If the FS player was matched to a SF player (id={sf_player_id}), "
    ...
    available_player_csvs_sorted_by_timedelta_to_match = sorted(
        sf_player_available_csv_files,
        key=lambda x: abs(x[1].replace(tzinfo=match_datetime.tzinfo) - match_datetime),
    )  # sort
    available_player_csvs_sorted_by_timedelta_to_match = [
        x
        for x in available_player_csvs_sorted_by_timedelta_to_match
        if abs(x[1].replace(tzinfo=match_datetime.tzinfo) - match_datetime) < settings.MAX_TIMEDELTA_SF_PLAYER_SKILL
    ]  # filter

    if len(available_player_csvs_sorted_by_timedelta_to_match) == 0:
        print(
            f"There are no available player CSV files within the timedelta range for player {sf_player_id}. "
            f"Imputing..."
        )
        # TODO manual output check: count how many such players without CSV data are there
        global_instance.mp7_team_strength_imitated_skills_as_no_CSV_data[comp_id] += 1
    ...
    while len(available_player_csvs_sorted_by_timedelta_to_match) > 0:
        ...
        if corresponding_player_data_dict[0] != datetime_of_csv_data_file:
            raise ValueError(
                f"Datetime of the latest CSV file available for the player that is older than the match "
    ...
    for index, (skill_name, value) in enumerate(collected_player_skills.items()):
        if value == -1:
            ...
            global_instance.mp7_SKILLS_team_strength_imitated_skills_as_no_data[comp_id] += 1

    if len(collected_player_skills) != len(settings.PLAYER_SKILLS):
        raise ValueError(
            f"Found {len(collected_player_skills)} skill values for SF player (id={sf_player_id}), but "
...
def match_af_team_to_fs_team(af_team_name, fs_teams_in_comp_season):
    ...
    print(
        f"\t\t\t\t\tAF team matched to FS team: [{af_team_name}] [{best_fs_match['cleanName']}] "
...
def get_fs_match_lineups(curr_match):  # for both home and away teams!
    ...
    home_team_fs_players_in_comp_season = [
        x["fs_players"]
        for x in curr_match.home_team.players_in_regular_comp_season
        if curr_match.comp == x["comp"] and curr_match.season == x["season"]
    ]

    if len(home_team_fs_players_in_comp_season) > 1:  # error
        raise ValueError(
            f"Multiple FS home team comp season lineups found for match between "
            f"{curr_match.home_team.name} and {curr_match.away_team.name} "
            f"({curr_match.datetime}) - ERROR!"
        )

    if (
        len(curr_match.home_fs_team_lineup) > 0
        or len(home_team_fs_players_in_comp_season) == 0
        or (curr_match.home_team_lineup is None or len(curr_match.home_team_lineup) == 0)
    ):  # empty
        ...
        if len(curr_match.home_fs_team_lineup) != 11:
            raise ValueError(
                f"There were {len(curr_match.home_fs_team_lineup)} matched home team FS players, "
                f"but the expected numbers of matches is 11"
            )

    # Away team
    away_team_fs_players_in_comp_season = [
        x["fs_players"]
        for x in curr_match.away_team.players_in_regular_comp_season
        if curr_match.comp == x["comp"] and curr_match.season == x["season"]
    ]

    if len(away_team_fs_players_in_comp_season) > 1:  # error
        raise ValueError(
            f"Multiple FS away team comp season lineups found for match between "
            f"{curr_match.home_team.name} and {curr_match.away_team.name} "
            f"({curr_match.datetime}) - ERROR!"
        )

    if (
        len(curr_match.away_fs_team_lineup) > 0
        or len(away_team_fs_players_in_comp_season) == 0
        or (curr_match.away_team_lineup is None or len(curr_match.away_team_lineup) == 0)
    ):  # empty
        ...
        if len(curr_match.away_fs_team_lineup) != 11:
            raise ValueError(
                f"There were {len(curr_match.away_fs_team_lineup)} matched away team FS players, "
                f"but the expected numbers of matches is 11"
            )
...
def get_fs_match_xg(curr_match):
    ...
    fs_match_ids = [
        x["fs_match_id"]
        for x in all_fs_matches_this_comp_season
        if x["fs_home_team_id"] == curr_match.home_team.fs_id
        and x["fs_away_team_id"] == curr_match.away_team.fs_id
        and x["season"] == curr_match.season
        and x["datetime"].year == curr_match.datetime.year
        and x["datetime"].month == curr_match.datetime.month
        and x["datetime"].day == curr_match.datetime.day
    ]

    if len(fs_match_ids) != 1:
        print(
            f"Found none, or multiple FS match IDs: [{fs_match_ids}] for a single match "
        ...
        (
            curr_match.home_team_pre_match_xg,
            curr_match.away_team_pre_match_xg,
            curr_match.total_pre_match_xg,
        ) = (-1, -1, -1)
        return

    fs_match_id = fs_match_ids[0]
    match_details_request_string_fs = (
        settings.FS_HOST + "/match?key=" + settings.FS_KEY + "&match_id=" + str(fs_match_id)
    )
    ...
    curr_match.home_team_xg = (
        float(fs_match_details_dict_comp_season["team_a_xg"])
        if float(fs_match_details_dict_comp_season["team_a_xg"]) > 0.001
        else -1
    )

    curr_match.away_team_xg = (
        float(fs_match_details_dict_comp_season["team_b_xg"])
        if float(fs_match_details_dict_comp_season["team_b_xg"]) > 0.001
        else -1
    )

    curr_match.total_xg = (
        float(fs_match_details_dict_comp_season["total_xg"])
        if float(fs_match_details_dict_comp_season["total_xg"]) > 0.001
        else -1
    )

    curr_match.home_team_pre_match_xg = (
        float(fs_match_details_dict_comp_season["team_a_xg_prematch"])
        if float(fs_match_details_dict_comp_season["team_a_xg_prematch"]) > 0.001
        else -1
    )

    curr_match.away_team_pre_match_xg = (
        float(fs_match_details_dict_comp_season["team_b_xg_prematch"])
        if float(fs_match_details_dict_comp_season["team_b_xg_prematch"]) > 0.001
        else -1
    )

    curr_match.total_pre_match_xg = (
        float(fs_match_details_dict_comp_season["total_xg_prematch"])
...
def match_fs_player_to_sf_players(fs_player, sf_players_with_same_dob, comp_id):
    ...
    print(
        f"Matched [{fs_player['fs_known_as']}] to [{highest_similarity_sf_player[2]}] "
        f"({highest_similarity_sf_player[1]}) with score \t\t{highest_similarity_score:.2f}"
    )

    global_instance = Global.get_instance()
    if highest_similarity_score < 65:
        global_instance.mp6_FS_SF_players_matching_potential_misses_couples[comp_id].append(
    ...
    if highest_similarity_score < settings.SIMILARITY_THRESHOLD_FS_SOFIFA:
        ...
        global_instance.mp6_team_strength_FS_SF_matching[comp_id] += 1
...
def add_imitated_player_skills(season, team_id, team_sf_players_skills, lineup_fs_positions, comp_id):
    ...
    sorted_possible_fs_positions = sort_positions_by_missing_count(
        possible_fs_positions, expected_position_occurrences, lineup_fs_positions
    )
    ...
    while lineup_length < 11:
        for position in sorted_possible_fs_positions:
            ...
            global_instance = Global.get_instance()
            global_instance.mp8b_team_strength_imitated_players_as_no_CSV_data[comp_id] += 1
    ...
    if lineup_length != 11:
        raise ValueError(
            f"Internal error occurred when adding imitated player skills: resulting lineup length equals "
...
def balance_goalkeeper_and_outfield_player_skills(season, team_id, team_skills, comp_id):
    global_instance = Global.get_instance()
    ...
    for idx, player in enumerate(team_skills):
        ...
        goalkeeper_mean = sum(player[i] for i in range(29, 33)) / 4  # avg of indices [29,33]
        ...
        if idx == 0 and goalkeeper_mean < outfield_mean:  # replace probable outfield player with a goalkeeper
            ...
            global_instance.mp9_team_strength_balancing_field_to_gk[comp_id] += 1
            ...
            if idx in [1, 2, 3, 4]:
                ...
                print(
                    f"Balancing player skills - replacing goalkeeper with defender: "
                    f"{team_skills[idx]}\n->\n{imitated_gk_skills}"
                )
                # TODO code: Results always in replacing goalkeeper with a defender, ...
                # TODO: ...never with midfielders or forwards
                team_skills[idx] = imitated_gk_skills
                global_instance.mp9_team_strength_balancing_gk_to_def[comp_id] += 1
            elif idx in [5, 6, 7, 8]:
                imitated_gk_skills = get_imitated_player_skills(season, team_id, "Midfielder")
                print(
                    f"Balancing player skills - replacing goalkeeper with midfielder: "
                    f"{team_skills[idx]}\n->\n{imitated_gk_skills}"
                )
                team_skills[idx] = imitated_gk_skills
                global_instance.mp9_team_strength_balancing_gk_to_mid[comp_id] += 1
            elif idx in [9, 10]:
                imitated_gk_skills = get_imitated_player_skills(season, team_id, "Forward")
                print(
                    f"Balancing player skills - replacing goalkeeper with forward: "
                    f"{team_skills[idx]}\n->\n{imitated_gk_skills}"
                )
                team_skills[idx] = imitated_gk_skills
                global_instance.mp9_team_strength_balancing_gk_to_att[comp_id] += 1
            else:
                raise ValueError(
...
def sort_positions_by_missing_count(possible_fs_positions, expected_position_occurrences, lineup_fs_positions):
    ...
    position_diff = {
        pos: expected_position_occurrences.get(pos, 0) - actual_counts.get(pos, 0) for pos in possible_fs_positions
    }
...
def get_most_frequent_string(list_of_strings):
    if not list_of_strings:
        raise ValueError("Cannot determine most frequent string from empty list of strings")
