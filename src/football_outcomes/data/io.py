    ...
    try:
        with open(file_name, mode="r", newline="", encoding="utf-8") as file:
            ...
            for row in reader:
                ...
                match.comp = next(
                    (comp for comp in global_instance.all_comps if comp.id == int(row["comp_id"])),
                    None,
                )
                ...
                if away_team is None:
                    raise Exception(
                        f"Failed to find an away team with ID " f"{int(row['away_team_id'])} to assign a match."
                    )
                ...
                match.home_fs_team_lineup = (
                    [
                        ...
                        for player in json.loads(home_fs_lineup_json)
                    ]
                    if home_fs_lineup_json != "null"
                    else []
                )
                ...
                match.away_fs_team_lineup = (
                    [
                        ...
                        for player in json.loads(away_fs_lineup_json)
                    ]
                    if away_fs_lineup_json != "null"
                    else []
                )
    ...
    for index, (file_date, filename) in enumerate(files_with_dates):
        ...
        with open(file_path, mode="r", encoding="utf-8") as csvfile:
            ...
            for row_num, row in enumerate(reader, start=2):  # start=2 to account for header
                ...
                for attr_name, attr_type in attributes:
                    ...
                    if raw_value == "":
                        if attr_name in [
                            ...
                            "full_name",
                            "dob",
                        ]:  # These can't be missing!
                            raise ValueError(
                                f"Attributes player_id, name, full_name and dob cannot be missing! "
                        ...
                        try:
                            ...
                            if attr_name in [
                                ...
                                raise ValueError(
                                    f"Attributes player_id, name, full_name and dob must be parsed! "
                ...
                name = (
                    player_data["name"].split(sep=" - FIFA")[0]
                    if "FIFA" in player_data["name"]
                    else player_data["name"].split(sep=" -")[0]
                )
            ...
            # DEBUG PRINT
            print(
                f"[6] {num_players_skipped_this_csv} player rows were skipped for this CSV ({filename}) "
