...
class Match:
            ...
            for season in seasons:
                ...
                if existing is not None and (comp.id, season) in existing:
                    ...
                    request_string = (
                        ...
                        + str(comp.id)
                        + "&from="
                        + latest_match_datetime.strftime("%Y-%m-%d")
                        + "&to="
                ...
                print(
                    f"[3] {len(data_fixtures['response'])} matches were found in comp {comp.name}"
                ...
                for fixture in data_fixtures["response"]:
                    ...
                    new_match.home_team_shots_inside_box = new_match.get_stats_value(
                        data_stats, "Shots insidebox", "home"
                    )
                    new_match.away_team_shots_inside_box = new_match.get_stats_value(
                        data_stats, "Shots insidebox", "away"
                    )
                    ...
                    new_match.home_team_ball_possession = new_match.get_stats_value(
                        data_stats, "Ball Possession", "home"
                    )
                    new_match.away_team_ball_possession = new_match.get_stats_value(
                        data_stats, "Ball Possession", "away"
                    )
                    ...
                    if len(data_lineups) == 0:
                        if new_match.round.is_regular:  # printing this out for irregular matches is waste of stdout
                            print(
                                f"\tLineups missing for both teams in a regular match between "
                                f"{new_match.home_team.name} and {new_match.away_team.name} ({new_match.datetime})"
                            )
                    else:
                        if "startXI" in data_lineups[0] and all(
                            isinstance(x["player"]["id"], int) for x in data_lineups[0]["startXI"]
                        ):
                            ...
                            new_match.home_team_lineup = []
                            print(
                                f"\tLineups missing for a home team in match between {new_match.home_team.name} and "
                                f"{new_match.away_team.name} played at {new_match.datetime}"
                            )
                        if (
                            len(data_lineups) > 1
                            and "startXI" in data_lineups[1]
                            and all(isinstance(x["player"]["id"], int) for x in data_lineups[1]["startXI"])
                        ):
                            ...
                            new_match.away_team_lineup = []
                            print(
                                f"\tLineups missing for an away team in match between {new_match.home_team.name} and "
                                f"{new_match.away_team.name} played at {new_match.datetime}"
                            )
    ...
    def get_stats_value(self, stats, stat_name, home_away):
        ...
        if len(stats) != 2:
            raise Exception(
                f"Fixture statistics response expected to contain info for exactly two matches."
    ...
    def calculate_relative_match_position_in_country_season(self, season):
        ...
        start_date = global_instance.start_end_dates_per_country_season[self.country][season]["start"].replace(
            tzinfo=self.datetime.tzinfo
        )
        end_date = global_instance.start_end_dates_per_country_season[self.country][season]["end"].replace(
            tzinfo=self.datetime.tzinfo
        )
        ...
        if start_date <= self.datetime <= end_date + timedelta(days=days_safe_margin):
            print(
                f"___WARNING: Found match between {self.home_team.name} and {self.away_team.name} played at "
        ...
        print(
            f"ERROR: Found match between {self.home_team.name} and {self.away_team.name} played at "
            f"{str(self.datetime)} not fitting into the expected timedelta range "
            f"[{str(start_date)},{str(end_date)}] - too late"
        )
        return None
    ...
    def calculate_match_features(self):
        ...
        # Create new instance
        new_match_features = MatchFeatures(
            self.comp.id,
            normalized_season,
            self.home_team.id,
            self.away_team.id,
        ...
        new_match_features.home_match_load_per_day_last_10_days = 1.0 - feature_ut.get_match_load_per_day_last_n(
            self, 10, "home"
        )
        new_match_features.home_match_load_per_day_last_25_days = 1.0 - feature_ut.get_match_load_per_day_last_n(
            self, 25, "home"
        )
        new_match_features.away_match_load_per_day_last_10_days = 1.0 - feature_ut.get_match_load_per_day_last_n(
            self, 10, "away"
        )
        new_match_features.away_match_load_per_day_last_25_days = 1.0 - feature_ut.get_match_load_per_day_last_n(
            self, 25, "away"
        )
        ...
        new_match_features.home_avg_xg_total_last_20 = feature_ut.get_avg_stat_value_last_n(
            self, 20, "home", "Total xG"
        )
        new_match_features.away_avg_xg_total_last_5 = feature_ut.get_avg_stat_value_last_n(self, 5, "away", "Total xG")
        new_match_features.away_avg_xg_total_last_20 = feature_ut.get_avg_stat_value_last_n(
            self, 20, "away", "Total xG"
        )

        new_match_features.home_avg_pre_match_xg_last_5 = feature_ut.get_avg_stat_value_last_n(
            self, 5, "home", "Team pre-m xG"
        )
        new_match_features.home_avg_pre_match_xg_last_20 = feature_ut.get_avg_stat_value_last_n(
            self, 20, "home", "Team pre-m xG"
        )
        new_match_features.away_avg_pre_match_xg_last_5 = feature_ut.get_avg_stat_value_last_n(
            self, 5, "away", "Team pre-m xG"
        )
        new_match_features.away_avg_pre_match_xg_last_20 = feature_ut.get_avg_stat_value_last_n(
            self, 20, "away", "Team pre-m xG"
        )

        new_match_features.home_avg_pre_match_xg_total_last_5 = feature_ut.get_avg_stat_value_last_n(
            self, 5, "home", "Total pre-m xG"
        )
        new_match_features.home_avg_pre_match_xg_total_last_20 = feature_ut.get_avg_stat_value_last_n(
            self, 20, "home", "Total pre-m xG"
        )
        new_match_features.away_avg_pre_match_xg_total_last_5 = feature_ut.get_avg_stat_value_last_n(
            self, 5, "away", "Total pre-m xG"
        )
        new_match_features.away_avg_pre_match_xg_total_last_20 = feature_ut.get_avg_stat_value_last_n(
            self, 20, "away", "Total pre-m xG"
        )
        ...
        new_match_features.home_avg_shots_on_target_last_5 = feature_ut.get_avg_stat_value_last_n(
            self, 5, "home", "Shots on Goal"
        )
        new_match_features.home_avg_shots_on_target_last_20 = feature_ut.get_avg_stat_value_last_n(
            self, 20, "home", "Shots on Goal"
        )
        new_match_features.away_avg_shots_on_target_last_5 = feature_ut.get_avg_stat_value_last_n(
            self, 5, "away", "Shots on Goal"
        )
        new_match_features.away_avg_shots_on_target_last_20 = feature_ut.get_avg_stat_value_last_n(
            self, 20, "away", "Shots on Goal"
        )

        new_match_features.home_avg_total_shots_last_5 = feature_ut.get_avg_stat_value_last_n(
            self, 5, "home", "Total Shots"
        )
        new_match_features.home_avg_total_shots_last_20 = feature_ut.get_avg_stat_value_last_n(
            self, 20, "home", "Total Shots"
        )
        new_match_features.away_avg_total_shots_last_5 = feature_ut.get_avg_stat_value_last_n(
            self, 5, "away", "Total Shots"
        )
        new_match_features.away_avg_total_shots_last_20 = feature_ut.get_avg_stat_value_last_n(
            self, 20, "away", "Total Shots"
        )

        new_match_features.home_avg_shots_inside_box_last_5 = feature_ut.get_avg_stat_value_last_n(
            self, 5, "home", "Shots insidebox"
        )
        new_match_features.home_avg_shots_inside_box_last_20 = feature_ut.get_avg_stat_value_last_n(
            self, 20, "home", "Shots insidebox"
        )
        new_match_features.away_avg_shots_inside_box_last_5 = feature_ut.get_avg_stat_value_last_n(
            self, 5, "away", "Shots insidebox"
        )
        new_match_features.away_avg_shots_inside_box_last_20 = feature_ut.get_avg_stat_value_last_n(
            self, 20, "away", "Shots insidebox"
        )

        new_match_features.home_avg_corner_kicks_last_5 = feature_ut.get_avg_stat_value_last_n(
            self, 5, "home", "Corner Kicks"
        )
        new_match_features.home_avg_corner_kicks_last_20 = feature_ut.get_avg_stat_value_last_n(
            self, 20, "home", "Corner Kicks"
        )
        new_match_features.away_avg_corner_kicks_last_5 = feature_ut.get_avg_stat_value_last_n(
            self, 5, "away", "Corner Kicks"
        )
        new_match_features.away_avg_corner_kicks_last_20 = feature_ut.get_avg_stat_value_last_n(
            self, 20, "away", "Corner Kicks"
        )

        new_match_features.home_avg_ball_possession_last_5 = feature_ut.get_avg_stat_value_last_n(
            self, 5, "home", "Ball Possession"
        )
        new_match_features.home_avg_ball_possession_last_20 = feature_ut.get_avg_stat_value_last_n(
            self, 20, "home", "Ball Possession"
        )
        new_match_features.away_avg_ball_possession_last_5 = feature_ut.get_avg_stat_value_last_n(
            self, 5, "away", "Ball Possession"
        )
        new_match_features.away_avg_ball_possession_last_20 = feature_ut.get_avg_stat_value_last_n(
            self, 20, "away", "Ball Possession"
        )

        new_match_features.home_avg_passes_acc_last_5 = feature_ut.get_avg_stat_value_last_n(
            self, 5, "home", "Passes %"
        )
        new_match_features.home_avg_passes_acc_last_20 = feature_ut.get_avg_stat_value_last_n(
            self, 20, "home", "Passes %"
        )
        new_match_features.away_avg_passes_acc_last_5 = feature_ut.get_avg_stat_value_last_n(
            self, 5, "away", "Passes %"
        )
        new_match_features.away_avg_passes_acc_last_20 = feature_ut.get_avg_stat_value_last_n(
            self, 20, "away", "Passes %"
        )
        ...
        if self.round.is_regular:
            ...
            new_match_features.home_curr_position = table.get_curr_team_position_in_season_up_to_date(
                self.home_team.id, self.datetime
            )
            new_match_features.away_curr_position = table.get_curr_team_position_in_season_up_to_date(
                self.away_team.id, self.datetime
            )
        else:
        ...
        if self.round.is_regular:
            # DEBUG PRINT
            print(
                f"[7c] Going to calculate team strength for match between "
