    ...
    with open(comps_csv_path, "w", newline="", encoding="utf-8") as f:
        ...
        for comp in unique_comps:
            ...
            for item in comp.rounds_per_season:
                ...
                rps_json.append({"season": item["season"], "rounds": rounds_id_list})
...
def store_rounds(rounds_csv_path):
    ...
    fieldnames = ["round_id", "comp_id", "comp_name", "season", "name", "is_regular"]
    ...
    with open(matches_csv_path, mode="w", newline="", encoding="utf-8") as f:
        ...
        for match in global_instance.all_matches:
            ...
            row["home_team_shots_on_target"] = (
                match.home_team_shots_on_target if match.home_team_shots_on_target is not None else ""
            )
            row["away_team_shots_on_target"] = (
                match.away_team_shots_on_target if match.away_team_shots_on_target is not None else ""
            )
            row["home_team_total_shots"] = (
                match.home_team_total_shots if match.home_team_total_shots is not None else ""
            )
            row["away_team_total_shots"] = (
                match.away_team_total_shots if match.away_team_total_shots is not None else ""
            )
            row["home_team_shots_inside_box"] = (
                match.home_team_shots_inside_box if match.home_team_shots_inside_box is not None else ""
            )
            row["away_team_shots_inside_box"] = (
                match.away_team_shots_inside_box if match.away_team_shots_inside_box is not None else ""
            )
            row["home_team_corner_kicks"] = (
                match.home_team_corner_kicks if match.home_team_corner_kicks is not None else ""
            )
            row["away_team_corner_kicks"] = (
                match.away_team_corner_kicks if match.away_team_corner_kicks is not None else ""
            )
            row["home_team_ball_possession"] = (
                match.home_team_ball_possession if match.home_team_ball_possession is not None else ""
            )
            row["away_team_ball_possession"] = (
                match.away_team_ball_possession if match.away_team_ball_possession is not None else ""
            )
...
def load_matches(matches_csv_path):
    ...
    with open(matches_csv_path, "r", encoding="utf-8") as f:
        ...
        for row in reader:
            ...
            m.home_team_shots_on_target = (
                int(row["home_team_shots_on_target"]) if row["home_team_shots_on_target"] else None
            )
            m.away_team_shots_on_target = (
                int(row["away_team_shots_on_target"]) if row["away_team_shots_on_target"] else None
            )
            ...
            m.home_team_shots_inside_box = (
                int(row["home_team_shots_inside_box"]) if row["home_team_shots_inside_box"] else None
            )
            m.away_team_shots_inside_box = (
                int(row["away_team_shots_inside_box"]) if row["away_team_shots_inside_box"] else None
            )
            ...
            m.home_team_ball_possession = (
                float(row["home_team_ball_possession"]) if row["home_team_ball_possession"] else None
            )
            m.away_team_ball_possession = (
                float(row["away_team_ball_possession"]) if row["away_team_ball_possession"] else None
            )
            ...
            m.home_elo_before_match_not_normalized = (
                float(row["home_elo_before_match_not_normalized"])
                if row["home_elo_before_match_not_normalized"]
                else None
            )
            m.away_elo_before_match_not_normalized = (
                float(row["away_elo_before_match_not_normalized"])
                if row["away_elo_before_match_not_normalized"]
                else None
            )
...
def load_all_matches_data():
    # 1) load comps
    load_comps(settings.ALL_COMPS_CSV)
    # 2) load rounds (which references comps)
    load_rounds(settings.ALL_ROUNDS_CSV)
    # 3) load teams
    load_teams(settings.ALL_TEAMS_CSV)
    # 4) load matches (which references comps, rounds, and teams)
    load_matches(settings.ALL_MATCHES_CSV)
